...
import { Card, Col, Row, Typography, Space, Button, Badge } from 'antd';
import { CloudOutlined, ArrowRightOutlined, CompassOutlined } from '@ant-design/icons';
import { useNavigate } from 'react-router-dom';
import { Leaf, PawPrint, Fish, Settings, Link as LinkIcon, Package, Sun, CloudRain } from 'lucide-react';
    ...
    <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in duration-700">
      ...
      <div className="flex justify-between items-end mb-2">
        <div className="space-y-1">
          ...
          <Title level={2} className="!mb-0">Chào bạn, <span className="text-green-600">Trần Đức Anh</span>! 👋</Title>
      ...
      {/* Top Row: Weather & Quick Access */}
      <Row gutter={[24, 24]}>
        {/* Weather Card */}
        <Col xs={24} lg={11}>
          <Card bordered={false} className="weather-gradient h-full !p-2">
            <div className="flex justify-between items-start mb-10">
              <Badge status="processing" color="#22c55e" text={<span className="font-bold text-gray-800">Dự báo thời tiết Mai Dịch</span>} />
              <Text className="text-[10px] font-bold text-green-600 bg-green-100 px-2 py-0.5 rounded-full uppercase tracking-tighter">Live Update</Text>
            </div>
            ...
            <div className="flex items-center justify-between gap-4">
              <div className="flex items-center gap-6">
                <div className="relative">
                   ...
                   <CloudOutlined className="text-7xl text-blue-400 relative z-10" />
                </div>
                <div className="flex flex-col">
                  <span className="text-7xl font-bold tracking-tighter text-gray-900 leading-none">29°</span>
                  <span className="text-lg text-gray-800 font-bold ml-1">Có Mây</span>
                </div>
              </div>
              
              <div className="bg-white/60 backdrop-blur-sm p-5 rounded-2xl border border-white space-y-3 min-w-[140px]">
                <div className="flex justify-between items-center gap-4">
                  <Text className="text-gray-400 text-xs font-bold uppercase">Độ ẩm</Text>
                  <Text className="text-gray-800 font-bold">79%</Text>
                </div>
                <div className="flex justify-between items-center gap-4">
                  <Text className="text-gray-400 text-xs font-bold uppercase">Mưa</Text>
                  <Text className="text-gray-800 font-bold">0%</Text>
                </div>
                <div className="flex justify-between items-center gap-4">
                   <Text className="text-gray-400 text-xs font-bold uppercase">Gió</Text>
                   <Text className="text-gray-800 font-bold text-xs uppercase">5km/h</Text>
                </div>
              </div>
            </div>
            ...
            <div className="mt-12 flex items-center justify-between pt-4 border-t border-gray-100">
               <Space>
                  <div className="flex flex-col items-center">
                    <Text className="text-[10px] text-gray-400 font-bold uppercase">Ngày mai</Text>
                    <Sun className="w-4 h-4 text-yellow-500 my-1" />
                    <Text className="text-xs font-bold">32°</Text>
                  </div>
                  <div className="w-[1px] h-8 bg-gray-100 mx-2"></div>
                  <div className="flex flex-col items-center">
                    <Text className="text-[10px] text-gray-400 font-bold uppercase">Thứ 7</Text>
                    <CloudRain className="w-4 h-4 text-blue-400 my-1" />
                    <Text className="text-xs font-bold">26°</Text>
                  </div>
               </Space>
               <Button type="text" className="text-green-600 font-bold text-xs p-0 flex items-center gap-1">Chi tiết 7 ngày <ArrowRightOutlined className="text-[10px]" /></Button>
            </div>
          </Card>
        </Col>
