...
const updateAllCropSchemasToTCVN = async () => {
  try {
    console.log('🌱 Cập nhật tất cả schemas cây trồng thành format TCVN chuẩn...');

    // Lấy tất cả schemas cây trồng (category: 'trongtrot')
    const cropSchemas = await FormSchema.find({ 
      category: { $in: ['trongtrot', 'huuco_caytrong'] }
    });

    console.log(`📋 Tìm thấy ${cropSchemas.length} schemas cây trồng cần cập nhật:`);
    
    cropSchemas.forEach((schema, index) => {
      console.log(`   ${index + 1}. ${schema.name} (${schema.category})`);
    });

    // Template TCVN chuẩn cho tất cả cây trồng
    const tcvnTables = [
      {
    ...
    for (const schema of cropSchemas) {
      try {
        // Cập nhật schema với tables TCVN chuẩn
        await FormSchema.findByIdAndUpdate(schema._id, {
          $set: {
            tables: tcvnTables,
            description: `${schema.description} - Cập nhật theo tiêu chuẩn TCVN với 5 bảng đầy đủ`
          }
        });
    ...
    console.log(`\n🎯 Hoàn thành! Đã cập nhật ${updatedCount}/${cropSchemas.length} schemas`);
    console.log('\n📋 Tất cả schemas cây trồng giờ đã có:');
    console.log('   1. Thông tin chung (10 trường)');
    console.log('   2. Đánh giá chỉ tiêu ATTP (6 trường)');
    console.log('   3. Nhật ký mua vật tư nông nghiệp (11 trường)');
    console.log('   4. Nhật ký thực hành sản xuất (21 trường)');
    console.log('   5. Nhật ký thu hoạch và bán sản phẩm (14 trường)');
