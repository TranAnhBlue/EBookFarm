...
const createVietGAHPPoultrySchema = async () => {
  try {
    console.log('🐔 Tạo schema nhật ký chăn nuôi gà thịt theo VietGAHP...');

    // Schema nhật ký chăn nuôi gà theo VietGAHP chuẩn
    const vietgahpPoultrySchema = new FormSchema({
      name: 'Gia cầm',
      description: 'Sổ nhật ký ghi chép chăn nuôi gà theo tiêu chuẩn VietGAHP - Quyết định số 4653/QĐ-BNN-CN',
      category: 'channuoi',
      tables: [
        {
          ...
          fields: [
            ...
            { name: 'tenGiong', label: 'Tên giống', type: 'select', options: ['Gà Ross 308', 'Gà Cobb 500', 'Gà Arbor Acres', 'Gà Hubbard', 'Gà ta', 'Khác'], required: true },
        ...
        },
        {
          ...
          fields: [
            ...
            { name: 'loaiVaccin', label: 'Loại vaccin', type: 'select', options: ['Newcastle', 'Gumboro', 'Bronchitis', 'Fowl Pox', 'Avian Influenza', 'Khác'], required: false }
          ]
        },
        {
        ...
        },
        {
          ...
          fields: [
            ...
            { name: 'noiDungThucHien', label: 'Nội dung thực hiện', type: 'select', options: ['Tiêm phòng', 'Điều trị bệnh', 'Tẩy giun', 'Vitamin', 'Khác'], required: true },
        ...
        },
        {
          ...
          fields: [
            ...
            { name: 'phuongPhapSatTrung', label: 'Phương pháp', type: 'select', options: ['Phun', 'Rắc', 'Ngâm', 'Pha nước', 'Khác'], required: true },
    ...
    // Tìm schema "Gia cầm" hiện có để cập nhật
    const existingSchema = await FormSchema.findOne({ name: 'Gia cầm', category: 'channuoi' });
    
    if (existingSchema) {
      // Cập nhật schema hiện có
      await FormSchema.findByIdAndUpdate(existingSchema._id, {
        description: vietgahpPoultrySchema.description,
        tables: vietgahpPoultrySchema.tables
      });
      console.log('✅ Đã cập nhật schema "Gia cầm" hiện có');
      console.log('📋 Schema ID:', existingSchema._id);
    } else {
      // Tạo schema mới nếu chưa có
      await vietgahpPoultrySchema.save();
      console.log('✅ Đã tạo schema "Gia cầm" mới');
      console.log('📋 Schema ID:', vietgahpPoultrySchema._id);
    }
    
    console.log('📊 Số bảng:', vietgahpPoultrySchema.tables.length);
    
    // Hiển thị thông tin các bảng
    vietgahpPoultrySchema.tables.forEach((table, index) => {
      console.log(`   ${index + 1}. ${table.tableName} (${table.fields.length} trường)`);
    });

    process.exit(0);
  } catch (error) {
    console.error('❌ Lỗi tạo/cập nhật schema:', error);
    process.exit(1);
  }
};

createVietGAHPPoultrySchema();