...
// Base table definition template for Tôm (Shrimp)
const baseTables = [
  {
...
// Helper to make custom schema dynamically
const makeAquacultureSchema = (name, typeWordLower, typeWordLowerNoAccent, typeWordUpperWithAccent, typeWordUpperNoAccent) => {
  const tables = JSON.parse(JSON.stringify(baseTables));

  tables.forEach(table => {
    table.tableName = table.tableName
      .replace(/tôm/g, typeWordLower)
      .replace(/Tôm/g, typeWordUpperWithAccent);

    table.fields.forEach(field => {
      // Replace field names (No accents for clean DB keys)
      field.name = field.name
        .replace(/Tom/g, typeWordUpperNoAccent)
        .replace(/tom/g, typeWordLowerNoAccent);

      // Replace labels (With accents for beautiful UI)
  ...
  return {
    name,
    category: 'thuysan',
    description: `Sổ nhật ký nuôi ${typeWordLower} theo tiêu chuẩn VietGAP`,
    tables
  };
};
...
const run = async () => {
  try {
    ...
    const schemasToCreate = [
      makeAquacultureSchema('Tôm', 'tôm', 'tom', 'Tôm', 'Tom'),
      makeAquacultureSchema('Cá', 'cá', 'ca', 'Cá', 'Ca'),
      makeAquacultureSchema('Cua', 'cua', 'cua', 'Cua', 'Cua')
    ];
    ...
    for (const schemaData of schemasToCreate) {
      ...
      if (schema) {
        console.log(`Schema "${schemaData.name}" already exists, updating...`);
        schema.tables = schemaData.tables;
        schema.description = schemaData.description;
        schema.category = schemaData.category;
        await schema.save();
        console.log(`Schema "${schemaData.name}" updated successfully!`);
      } else {
        console.log(`Creating new schema "${schemaData.name}"...`);
        schema = new FormSchema(schemaData);
        await schema.save();
        console.log(`Schema "${schemaData.name}" created successfully!`);
      }
    }
